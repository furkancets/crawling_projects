...
for n in lst:
    url = URLLIST.crawler_fifth_one_url + str(n)
    print(url)
    ...
    for pr in main_page_for_link:
        ...
        detail = sess.get(URLLIST.crawler_fifth_two_url + str(pr_link))
        product_url = URLLIST.crawler_fifth_two_url + str(pr_link)
        print(product_url)
        ...
        if product_picture_1 is not None:
            ...
            product_picture = URLLIST.crawler_fifth_two_url + str(str_image)
        ...
        df.to_excel('farma_reciept.xlsx', index=False)
