...
for n in lst:
    url = URLLIST.crawler_three_one_url + str(n)
    print(url)
    ...
    for pr in main_page_for_link:
        ...
        detail = sess.get(URLLIST.crawler_three_two_url + str(pr_link))
        product_url = URLLIST.crawler_three_two_url + str(pr_link)
        ...
        df.to_excel('ebaby.xlsx', index=False)
